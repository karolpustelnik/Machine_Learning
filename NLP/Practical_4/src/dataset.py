...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #1. 
        rand_truncation = random.randint(4, self.block_size * 7/8)
        if len(document) > 4:
            document = document[:rand_truncation]
        #2.
        masked_len = random.randint(int(1 / 16 * len(document)), int(7/ 16 * len(document))) # EX = 1/4 truncated document
        prefix_len = random.randint(1, len(document) - masked_len - 1) #make sure prefix_len is at least 1. I substract 1 to make sure we leave something for suffix_len (suffix_len will always be at least 1)
        prefix = document[:prefix_len]
        #suffix len is unequivocal determined by prefix_len and masked_len
        ...
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.PAD_CHAR * (
            self.block_size - len(document) - 2) # to make sure masked_string is of length block_size, i add as many PAD_CHAR as there is left to fill: 
        #block_size - (len_prefix + len(masked_content) + len(suffix) - len(MASK_CHAR) - len(MASK_CHAR) = block_size - len(document) - 2
        #4.
