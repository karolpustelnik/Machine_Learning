...
def naive_softmax_loss_and_gradient(
    ...
    vocab_size, embedding_dim = outsideVectors.shape
    x = np.matmul(outsideVectors, centerWordVec)
    #x = x.reshape((vocab_size, 1))  #(vocab_size, 1)
    #print(x)
    y_hat = softmax(x)       #(vocab_size, 1)
    y_hat = y_hat.reshape((vocab_size, 1)) 
    ...
    gradCenterVec = np.matmul(np.transpose(outsideVectors), delta)
...
def neg_sampling_loss_and_gradient(
    ...
    sig_output = sigmoid(np.dot(u_o,centerWordVec))
    loss += -np.log(sig_output)
    gradCenterVec = u_o*(sig_output-1)
    gradOutsideVecs = np.zeros(outsideVectors.shape)   # create empty GradOutsideVencs,

    gradOutsideVecs[outsideWordIdx] = centerWordVec*(sig_output-1)

    for s in range(K):
        idx = indices[s+1]   #omit 1st index, since we already calculated gradient for it above
        u_k = outsideVectors[idx]
        sigm_output = sigmoid(-np.dot(u_k,centerWordVec))
        loss += -np.log(sigm_output)
        gradCenterVec += u_k*(1-sigm_output)
        gradOutsideVecs[idx] += centerWordVec*(1-sigm_output)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    current_center_word_vector = centerWordVectors[center_word_id]
    for outsideword in outsideWords:
        outsidewordIdx = word2Ind[outsideword]
        grads = word2vecLossAndGradient(current_center_word_vector, outsidewordIdx, outsideVectors, dataset)
        loss += grads[0]
        gradCenterVecs[center_word_id] += (grads[1]).reshape(len(current_center_word_vector),)
        gradOutsideVectors += grads[2] 
    ### END YOUR CODE
