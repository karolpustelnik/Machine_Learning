...
from frame_builders import FrameBuilder
...
class Motors:
    ...
    """
    def __init__(self, connection, motor2_multiplier=0.9847, verbose=False):
        self.connection = connection
    ...
    def command_motors(self, power_left, power_right):
        power_right = math.copysign(min(abs(power_right * self.motor2_multiplier), 100), power_right)
...
class Connection:
    ...
    def keep_stream_alive(self):
        ...
        if now > self.last_keepalive + MIN_KEEPALIVE_DELAY:
            ...
            self.sock_control.send(f"stream {self.my_ip} {self.recv_port} {self.camera_quality}\n".encode())
